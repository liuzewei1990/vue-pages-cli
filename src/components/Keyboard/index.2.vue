<template>
  <div class="Keyboard-box">
    <div class="numbers">
      ...
      @touchstart="handleNumber($event,item)"
...
export default {
  ...
  methods: {
    isMoneyRule(money) {
      ...
      //处理首字母为 . 的情况 改为 0.
      if (money === '.') this.values.unshift("0")

      //处理没有输入金额的情况 禁用支付按钮
      this.payDisable = !Boolean(Number(money))

      //处理小数点【>=】两位的情况 禁用键盘
      let floatlast = money.split(".")[1];
      floatlast && (this.btnDisable = floatlast.length >= 2);
    ...
    handleNumber(e, item) {
      ...
      //处理有多个小数点的情况
      if (number === "." && this.values.indexOf(number) !== -1) return;

      //处理首位纯为0的情况
      if (this.values.join("") === "0") this.handleDelete();

      //处理最大金额
      if (Number(this.values.join("") + number) > this.maxMoney) return;
